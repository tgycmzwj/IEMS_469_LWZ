...
Experience=collections.namedtuple('Experience',field_names=['state','action','prob','reward','done','new_state'])
#buffer
...
def select_action(state):
    ...
    return action.item(),torch.unsqueeze(torch.log(probs[0][action]),0)
...
def cal_loss(episodes_buffer):
    ...
    for episode in episodes_buffer.buffer:
        R=0
        returns, policy_loss, buffer_rewards, buffer_prob = [], [], [], []
        for step in episode:
        ...
        for log_prob, R in zip(buffer_prob, returns):
...
while True:
    ...
    while not done:
        ...
        cur_episode.append(Experience(state,action,prob,new_reward,new_done,new_state))
    ...
    if episodes_buffer.__len__()==BATCH_SIZE:
        optimizer.zero_grad()
        policy_loss = cal_loss(episodes_buffer)
        #policy_loss = torch.cat(policy_loss).sum().to(device)
        policy_loss.backward()
        optimizer.step()
        episodes_buffer.clear()
        print('Episode {}\tLoss :{:.2f}\tLast reward: {:.2f}\tLast 20 Average reward: {:.2f}'.format(i_episode, policy_loss, ep_reward, running_reward))
